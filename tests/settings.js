...
(async () => {
  ...
  ok('the sea is louder at the beach than in the meadow', await p.evaluate(async () => {
    ...
    const beach = await read(5900, 3520);
    const meadow = await read(4100, 1660);
    return beach > 0.25 && meadow < 0.05;
  }));
  ok('crickets come out at night and not at noon', await p.evaluate(async () => {
    const run = (mins) => {
      GG.Time.minutes = mins;
      for (let i = 0; i < 120; i++) GG.Ambience.update(0.05, 'world', 4100, 1660);
  ...
  const beach = await settle('world', 5900, 3520);
  ok('at the beach you can hear the sea (' + beach.surf.gain + ')', beach.surf.gain > 0.01);

  const meadow = await settle('world', 5650, 1740);
  ...
  const indoors = await settle('house', 5650, 1740);
  ...
  const fade = await p.evaluate(() => {
    ...
    read('world', 5900, 3520, 10);
    const atBeach = GG.Ambience.beds.surf.level;
    const after1s = read('world', 5650, 1740, 1);
    const after6s = read('world', 5650, 1740, 5);
